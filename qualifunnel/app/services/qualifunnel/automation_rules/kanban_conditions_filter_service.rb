...
class Qualifunnel::AutomationRules::KanbanConditionsFilterService
  ...
  def perform
    ...
    Rails.logger.error "Error in Qualifunnel::AutomationRules::KanbanConditionsFilterService: #{e.message}"
  ...
  def evaluate_assignee_condition(operator, values)
    task_agent_ids = @task.agents.pluck(:id)
