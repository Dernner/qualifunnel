...
const close = () => { showMenu.value = false; emit('close'); };

const open = event => {
  triggerElement.value = event.currentTarget || event.target;
  if (showMenu.value) { close(); return; }
...
const onSelect = item => { emit('select', item); close(); };
</script>
...
<template>
  ...
  <ContextMenu
    ...
    @close="close"
  >
    <div class="w-48 overflow-hidden rounded-lg bg-n-alpha-3 backdrop-blur-[100px] border border-n-strong dark:border-n-strong p-2 shadow-lg cursor-default">
