...
export function useBoardModal(options = {}) {
  ...
  const openBoardModal = () => { showBoardModal.value = true; };
  const closeBoardModal = () => { showBoardModal.value = false; };

  const saveBoard = async data => {
    isSavingBoard.value = true;
    try {
      const newBoard = await store.dispatch('qualifunnel/kanban/createBoard', data);
      closeBoardModal();
      if (onSuccess) await onSuccess(newBoard);
      return newBoard;
    } catch (error) {
  ...
  return { showBoardModal, isSavingBoard, openBoardModal, closeBoardModal, saveBoard };
}
